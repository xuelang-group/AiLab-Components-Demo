...
GIT_REMOTE_ORIGIN=${SUANPAN_GIT_REMOTE_ORIGIN}
...
if [ -n "${GIT_REMOTE_ORIGIN}" ]; then
    git remote set-url --push origin ${GIT_REMOTE_ORIGIN}
fi

git flow init -d
if [ -n "${GIT_REMOTE_ORIGIN}" ]; then
    git branch --set-upstream origin/develop && git push --all && git push --tags
fi
